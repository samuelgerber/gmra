...
class NodeFactory{
  ...
  private:
    ...
    DenseMatrix<TPrecision> createMatrix(std::vector<int>
        &indices, FortranLinalg::DenseMatrix<TPrecision> &X,
  ...
  protected:
    ...
    FortranLinalg::DenseMatrix<TPrecision> computeSVD(std::vector<int>
        &indices, FortranLinalg::DenseMatrix<TPrecision> &X,
        FortranLinalg::DenseVector<TPrecision> &mean, int svdDim){
      ...
      //Do streaming svd for large matrices      
      else if(indices.size() * X.M() > STREAMING_THRESHOLD){
      }
      //Do randomized svd for smaller dimensions
  ...
  public:
    ...
    virtual void createNode(std::vector<int> &indices,
        FortranLinalg::DenseMatrix<TPrecision> &X,
        FortranLinalg::DenseVector::<TPrecision>
        &mean) = 0;
...
class FixedNodeFactory : public NodeFactory<TPrecision>{
  ...
  public:
    ...
    virtual void createNode(std::vector<int> &indices,
        FortranLinalg::DenseVector<TPrecision> Xin,
        FortranLinalg::DenseVector<TPrecision> &mean ){ 
...
//GMRANode subclass
template <typename TPrecision> 
class NormalNode : public GMRANodeBase<TPrecision>{
  public:
  ...
  public:
    ...
    IPCANode<TPrecision>(FortranLinalg::DenseMatrix<TPrecision> &Xi, std::vector<int>
      ...
      if(splitDir == PC){
        ...
        dir = DenseMatrix<TPrecision>( phi.M(), std::min( maxKidDim,
              (int) phi.N() )  );
        Linalg<TPrecision>::Zero(dir);

        DenseVector<TPrecision> tmp(dir.M());
        for(int k=0; k<dir.N(); k++){
          Linalg<TPrecision>::Zero(tmp);
          ...
          Linalg<TPrecision>::SetColumn(dir, k, tmp);
        }
        tmp.deallocate();
...
class IPCATreeConfiguration{
  public:    
    ...
    IPCATreeConfiguration(){
      stop = TOTAL_R2;
      epsilon = 0;
      splitStrategy = IPCANode<TPrecision>::MEAN;
      minPoints = 1;
      nf = new FixedNodeFactory<TPrecision>(1);
    ...
    void print(){
      ...
      std::cout << maxKidDim << std::endl;
      std::cout << stop << std::endl;
      std::cout << minPoints << std::endl;
      std::cout << splitStrategy << std::endl;
      std::cout << splitDirectionStrategy << std::endl;
    };
};
...
class IPCATree : public GMRATree<TPrecision>{
  ...
  public:
   typedef typename IPCATreeConfiguration<TPrecision>::StoppingCriterium StoppingCriterium;

  private:
    typedef typename IPCANode<TPrecision>::SplitStrategy SplitStrategy;
    typedef typename IPCANode<TPrecision>::SplitDirectionStrategy SplitDirectionStrategy;

    IPCATreeConfiguration<TPrecision> config;
    ...
    void buildTreeRecursive(IPCANode<TPrecision> *node, FortranLinalg::DenseMatrix<TPrecision>
      ...
      if( node->getPoints().size() <= std::max(config.minPoints, (unsigned int) 1 ) ){ 
        return;
      }
      if(config.stop == IPCATreeConfiguration<TPrecision>::TOTAL_R2 && relativeR2 <= config.epsilon){
        return;
      }
      if(config.stop == IPCATreeConfiguration<TPrecision>::NODE_R2 && R2 <= config.epsilon){
        return;
      }
      if(config.stop == IPCATreeConfiguration<TPrecision>::NODE_MSE && mse <= config.epsilon){
        return;
      }
      if(config.stop == IPCATreeConfiguration<TPrecision>::NODE_RADIUS && node->getL2Radius() <= config.epsilon){
        return;
      }
      if(config.stop == IPCATreeConfiguration<TPrecision>::RELATIVE_NODE_RADIUS && relativeRadius <= config.epsilon){
        return;
      }      
      if(config.stop == IPCATreeConfiguration<TPrecision>::MASS_RADIUS && massRadius <= config.epsilon){
        return;
      }
      ...
      for(int i=0; i< children.size(); i++){
        if(children[i].size() > 0){
          IPCANode<TPrecision> *n = new IPCANode<TPrecision>(X, children[i], *config.nf,
              config.splitDirectionStrategy, config.splitStrategy, config.maxKidDim);
          node->addChild(n, i);
  ...
  public:
    ...
    IPCATree(IPCATreeConfiguration<TPrecision> &c) : config(c) { 
    };
    ...
    void construct(TPrecision *X, int m, int n){
      ...
      //if(Xc.N() < nPoints){
      IPCANode<TPrecision> *root = new IPCANode<TPrecision>(Xd, all, *config.nf,
          config.splitDirectionStrategy, config.splitStrategy, config.maxKidDim);
      //}
      //else{
    ...
    void flatten( std::ofstream &file ){
      ...
      file.write( (char*) &config.epsilon, sizeof(TPrecision) ) ;
      unsigned int m = root->phi.M();
      file.write( (char*) &m, sizeof(unsigned int) ) ;   
      file.write( (char*) &config.minPoints, sizeof(unsigned int) );   
    ...
    void unflatten( std::ifstream &file ){
      ...
      file.read( (char*) &config.epsilon, sizeof(TPrecision) ) ;
      unsigned int m;   
      file.read( (char*) &m, sizeof(unsigned int) );   
      file.read( (char*) &config.minPoints, sizeof(unsigned int) );   
